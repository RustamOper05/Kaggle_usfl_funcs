...
def xgboost_clf(X, y, test, k_fold, params, patience):
    ...
    for train_index, test_index in k_fold.split(X, y):
        ...
        predsXGB.append(model.predict(test))

    return np.average(np.array(predsXGB), axis=0), predsXGB, modelsXGB
...
def cb_clf(X, y, test, k_fold, params, patience):
    ...
    return np.average(np.array(predsCB), axis=0), predsCB, modelsCB
...
def lgb_clf(X, y, test, k_fold, params, patience):
    ...
    return np.average(np.array(predsLB), axis=0), predsLB, modelsLB
...
def SLB_clf(X, y, test, k_fold, params):
    ...
    return np.average(np.array(predsSGB), axis=0), predsSGB, modelsSGB
...
def tab_net_clf(X, y, test, k_fold, params, patience):
    ...
    for train_index, test_index in k_fold.split(X):
        ...
        predictions_array.append(clf.predict(test.values))
